alter table "public"."insights_ia" drop constraint "insights_ia_type_check";

drop index if exists "public"."idx_insights_global";

alter table "public"."insights_ia" add constraint "insights_ia_type_check" CHECK ((type = ANY (ARRAY['Opportunité extension'::text, 'Risque churn'::text, 'Moment recommandation'::text, 'Contact oublié'::text, 'Upsell détecté'::text, 'Tendance identifiée'::text, 'Pipeline faible'::text, 'Facture impayée'::text]))) not valid;

alter table "public"."insights_ia" validate constraint "insights_ia_type_check";

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.get_dashboard_metrics(p_user_id uuid, p_entreprise_id uuid DEFAULT NULL::uuid)
 RETURNS json
 LANGUAGE plpgsql
